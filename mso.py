...
class aciDB:
    ...
    def login(self, uid, pwd, apic):
        self.apic = str(apic)
        login_url = self.apic + '/api/v1/auth/login'
        ...
        try:
            post_resp = self.mysession.post(login_url, json=data, verify=False)
            post_resp_data = json.loads(post_resp.text)
            if post_resp.ok:
                self.msotoken = post_resp_data['token']
                print 'mso token is ' , self.msotoken
                return self.msotoken
            else:
    ...
    def get_schema(self):
        get_url = self.apic + '/api/v1/schemas'
        resp = self.mysession.get(get_url, verify=False, headers = {'Authorization': 'Bearer ' + self.msotoken})
...
apic_dict = {
    'mso': ['box1', 'mso', '198.18.135.70', '198.18.135.70']
}
...
if __name__ == '__main__':
    serve(app, host='0.0.0.0', port='7070', url_scheme='http')
